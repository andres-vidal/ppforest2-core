...
static json load_golden(std::string const& dataset, std::string const& slug) {
...
static void compare_predictions(json const& actual, json const& expected) {
...
static void compare_float_array(json const& actual, json const& expected, double tol, std::string const& name) {
  ASSERT_EQ(actual.size(), expected.size()) << name << " size mismatch";
  for (size_t i = 0; i < expected.size(); ++i) {
    EXPECT_NEAR(actual[i].get<double>(), expected[i].get<double>(), tol) << name << "[" << i << "]";
  }
}

static void compare_confusion_matrix(json const& actual, json const& expected) {
